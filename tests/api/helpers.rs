...
pub async fn spawn_app() -> TestApp {
    ...
    let mut configuraiton = get_configuration().expect("Failed to read configuration.");
    configuraiton.database.database_name = Uuid::new_v4().to_string();
    ...
    let timeout = configuraiton.email_client.timeout();
    let email_client = EmailClient::new(
        configuraiton.email_client.base_url,
        sender_email,
        configuraiton.email_client.authorization_token,
        timeout
    );
